...
def show_link(port: int = 8888, *, _url: "str | None" = None):
    ...
    *_url* is an optional pre-fetched proxy URL; pass it to avoid a second eval_js round-trip.
    ...
    <div style="display: inline-block; padding: 20px; background: #ffffff; border: 2px solid #000000;
        ...
        <a href="{url}" onclick="
            if (window.google && google.colab && google.colab.kernel && google.colab.kernel.proxyPort) {{
                google.colab.kernel.proxyPort({port}, {{cache: true}})
                    .then(function(u){{ window.open(u, '_blank'); }})
                    .catch(function(){{ window.open(this.href, '_blank'); }}.bind(this));
                return false;
            }}
            var w = window.open(this.href, '_blank'); if (!w) {{ return true; }} return false;"
